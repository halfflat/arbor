...
int main(int argc, char** argv) {
    try {
        ...
        std::vector<arb::trace_vector<double>> voltage(decomp.num_local_cells);

        // Now attach the sampler at probe_id, with sampling schedule sched, writing to voltage
        unsigned j=0;
        for (auto g : decomp.groups) {
            for (auto i : g.gids) {
                sim.add_sampler(arb::one_probe({i, 0}), sched, arb::make_simple_sampler(voltage[j++]));
            }
        }
...
void write_trace_json(const std::vector<arb::trace_vector<double>>& trace, unsigned rank) {
    for (unsigned i = 0; i < trace.size(); i++) {
        ...
        if (!trace[i].empty()) {
            for (const auto &sample: trace[i][0]) {
