...
int main(int argc, char** argv) {
    try {
        ...
        if (root) {
            // If there were no samples at all, write an empty trace,
            if (voltage.empty()) write_trace_json({});
            else {
                // There should be only the one trace, as the probe was at a single location.
                assert(voltage.size()==1u);
                write_trace_json(voltage[0]);
            }
        }
